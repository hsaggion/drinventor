...
public class PDFXConn {
	...
	private static Logger logger = Logger.getLogger(PDFXConn.class);	

	private static final String serviceURL = "http://pdfx.cs.man.ac.uk/";
	...
	public static Map<String, String> processPDFfile(String inputFilePath, int timeout) {
		...
		if(fileSizeBytes > 5242800l) {
			logger.info("IMPOSSIBLE TO CONVERT FILE SINCE IT IS GREATER THAN 5Mb - file name " + file.getName() + " - file size: " + file.length());
		...
		String result = processPDF(data, timeout);
		retMap.put(file.getName().replace(".pdf", "") + "_PDFX.xml", result.toString());

		logger.debug("End converting to XML file (PDFX): " + inputFilePath);

		return retMap;
	}

	/**
	 * Get a PDF file (max 5Mb) as a byte array and transform it to an XML annotated file by means of
	 * the PDFX Web Service (http://pdfx.cs.man.ac.uk/).
	 * 
	...
	public static String processPDF(byte[] inputBytes, int timeout) {
		...
		ByteArrayEntity entity = new ByteArrayEntity(inputBytes, ContentType.DEFAULT_BINARY);
		...
		try {
			...
			ResponseHandler<String> responseHandler = new ResponseHandler<String>() {
				...
				public String handleResponse(final HttpResponse response) throws ClientProtocolException, IOException {
					...
					if (status >= 200 && status < 300) {
						...
						return "PDFX ERROR - HTTP STATUS CODE: " + status;
					}
				}
			...
			logger.debug("PDFX parsing request processed in " + (endTime - startTime)  + " milliseconds with response: " + 
					((response != null) ? (response.length() > 15 ? response.substring(0, 15) : response) :"NULL") + " (sent file length: " + inputBytes.length + ")");
		} catch (ClientProtocolException e) {
			logger.error("PDFX processing exception / client protocol " + e.getMessage());
		} catch (Exception e) {
			logger.error("PDFX processing exception / Exception " + e.getMessage());
		}
	...
	public static int convertFilesAndStore(String fileOrDirFullPath, boolean enablePDFcompression, float compressionFactor, boolean recursiveDir, int timeout) {
		...
		if(parent.exists() && parent.isDirectory()) {
			...
			for(File file : children) {
				...
				convertedPDFcount += convertFilesAndStore(file.getAbsolutePath(), enablePDFcompression, compressionFactor, recursiveDir, timeout);
			}
		}
		else {
			if(parent.getName().toLowerCase().endsWith(".pdf")) {
				...
				try {
					String conversionResult = PDFXConn.processPDF(((compressed) ? compressedPDF : originalPDF), timeout);
					convertedPDFcount++;
